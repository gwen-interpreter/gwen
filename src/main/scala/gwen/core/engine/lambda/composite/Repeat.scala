            ...
            operation match {
              case "until" =>
                ...
                if (condSteps.isEmpty) {
                  engine.beforeStepDef(step, preCondStepDef, env.scopes)
                }
      ...
      if (condSteps.nonEmpty) {
        val steps = evaluatedStep.evalStatus match {
          case Failed(nanos, error) if (EvalStatus(condSteps.map(_.evalStatus)).status == StatusKeyword.Passed) => 
            ...
            engine.beforeStep(preCondStepDef, preStep, env.scopes)
            ...
            engine.afterStep(fStep, env.scopes)
            fStep :: condSteps
          case _ => 
            condSteps
        ...
        engine.afterStepDef(condStepDef, env.scopes)
        evaluatedStep.copy(
