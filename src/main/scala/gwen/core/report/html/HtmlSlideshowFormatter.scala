      ...
      val htmlPage = 
        html(lang := "en",
          ...
          body(
            ...
            HtmlReportFormatter.formatDetailStatusHeader(unit, result, rootPath, List(summaryCrumb, featureCrumb), Nil, false),
...
object HtmlSlideshowFormatter {
  ...
  private [report] def formatSlideshow(screenshots: List[File], rootPath: String): TypedTag[String] = {
    tag("center")(
        ...
        p(
          ...
          div(id := "controls-div", style := "display: none;",
            ...
            select(id := "frames-per-sec", title := "Frames per second..",
              for(i <- 1 to maxFramesPerSec(screenshots))
              yield
              option(i)
            ),
            " frames/sec"
