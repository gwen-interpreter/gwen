...
class SpecResult(
  ...
  private [model] lazy val scenarioCounts = 
  ...
  private [model] lazy val ruleCounts =
    StatusKeyword.countsByStatus(spec.rules.map(_.evalStatus))

  private [model] lazy val stepCounts = StatusKeyword.countsByStatus(spec.evalScenarios.flatMap(_.allSteps.map(_.evalStatus)))
