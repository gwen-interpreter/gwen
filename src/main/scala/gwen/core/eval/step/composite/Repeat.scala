            ...
            operation match {
              case "until" =>
                ...
                if (condSteps.isEmpty) {
                  engine.lifecycle.beforeStepDef(step, preCondStepDef, env.scopes)
                }
      ...
      if (condSteps.nonEmpty) {
        val steps = evaluatedStep.evalStatus match {
          case Failed(nanos, error) if (EvalStatus(condSteps.map(_.evalStatus)).status == StatusKeyword.Passed) => 
            ...
            engine.lifecycle.beforeStep(preCondStepDef, preStep, env.scopes)
            ...
            engine.lifecycle.afterStep(fStep, env.scopes)
            fStep :: condSteps
          case _ => 
            condSteps
        ...
        engine.lifecycle.afterStepDef(condStepDef, env.scopes)
        evaluatedStep.copy(
