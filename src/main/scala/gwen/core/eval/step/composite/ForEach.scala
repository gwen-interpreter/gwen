...
abstract class ForEach[T <: EvalContext](engine: EvalEngine[T], ctx: T) extends CompositeStep[T](engine, ctx) {
  ...
  def evaluateForEach[U](elements: ()=>Seq[U], element: String, parent: Identifiable, step: Step, doStep: String): Step = {
    ctx.withEnv { env =>
      ...
      engine.lifecycle.beforeStepDef(step, preForeachStepDef, env.scopes)
      val steps =
        elements() match {
          case Nil =>
                  ...
                  EvalStatus(acc.map(_.evalStatus)) match {
                    case status @ Failed(_, error)  =>
                      ...
                      if (failfast && !isSoftAssert) {
                        ...
                        engine.lifecycle.transitionStep(preForeachStepDef, foreachSteps(index), Skipped, env.scopes)
                      } else {
      ...
      engine.lifecycle.afterStepDef(foreachStepDef, env.scopes)
