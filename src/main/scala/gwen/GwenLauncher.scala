...
class GwenLauncher[T <: EvalContext](interpreter: GwenInterpreter[T]) extends LazyLogging {
  ...
  private def executeFeatureUnits(options: GwenOptions, featureStream: LazyList[FeatureUnit], ctxOpt: Option[T]): EvalStatus = {
    ...
    reportGenerators.foreach(_.init(interpreter.lifecycle))
    Try {
      if (options.parallel) {
      ...
      case Success(s) => 
        reportGenerators.foreach(_.close(interpreter.lifecycle, s.evalStatus))
        printSummaryStatus(s)
        s.evalStatus
      case Failure(f) => 
        reportGenerators.foreach(_.close(interpreter.lifecycle, Failed(System.nanoTime - start, f)))
        throw f
    }
  }
